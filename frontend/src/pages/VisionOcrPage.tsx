...
import {
  runVisionOcr, getVisionModels, VisionOcrResult,
} from "@/lib/api";
import {
  Upload, Loader2, FileText, RotateCcw, Copy, Check, Sparkles, Coins,
} from "lucide-react";
...
export default function VisionOcrPage({ dir }: Props) {
  ...
  const { data: modelsData } = useQuery({
    ...
    queryFn: () => getVisionModels().then(r => r.data.models),
    staleTime: Infinity,
  });

  const models = modelsData ?? ["claude-sonnet-4-6", "claude-haiku-4-5-20251001", "claude-opus-4-7"];

  const mutation = useMutation({
    mutationFn: (f: File) => runVisionOcr(f, model, prompt).then(r => r.data),
            ...
            <div className="h-12 px-4 border-b bg-white flex items-center gap-2 shrink-0">
              ...
              {result && (
                <span className="text-xs text-muted-foreground shrink-0 font-mono">
                  {result.model.split("-").slice(1, 3).join("-")} · {result.latency_ms} ms
                </span>
              )}
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-3">
              ...
              {phase === "running" && (
                <div className="flex flex-col items-center justify-center h-40 gap-3 text-muted-foreground">
                  ...
                  <p className="text-sm">{dir === "rtl" ? "Claude يقرأ الوثيقة..." : "Claude is reading the document..."}</p>
                </div>
              )}
            ...
            <div className="border-t bg-white px-6 py-5 space-y-3 shrink-0">
              ...
              >
                {isRunning
                  ...
                  : <><Sparkles className="w-4 h-4" />{dir === "rtl" ? "استخراج بـ Claude" : "Extract with Claude"}</>
                }
              </button>
