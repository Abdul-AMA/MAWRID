...
import {
  runGeminiOcr, getGeminiModels, VisionOcrResult,
} from "@/lib/api";
import {
...
export default function GeminiOcrPage({ dir }: Props) {
  ...
  const { data: modelsData } = useQuery({
    ...
    queryFn: () => getGeminiModels().then(r => r.data.models),
    staleTime: Infinity,
  });

  const models = modelsData ?? ["gemini-2.5-flash", "gemini-3-flash-preview"];

  const mutation = useMutation({
    mutationFn: (f: File) => runGeminiOcr(f, model, prompt).then(r => r.data),
          ...
          <div className="flex-1 flex items-center justify-center p-8">
            <label
              className={cn(
                ...
                dragging
                  ...
                  : "border-border hover:border-blue-400/60 hover:bg-muted/30 hover:shadow-sm"
              )}
            ...
            >
              <div className={cn(
                ...
                dragging ? "bg-blue-100 scale-110" : "bg-muted"
              )}>
                <Upload className={cn("w-10 h-10 transition-colors", dragging ? "text-blue-500" : "text-muted-foreground")} />
              </div>
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-3">
              ...
              {phase === "running" && (
                <div className="flex flex-col items-center justify-center h-40 gap-3 text-muted-foreground">
                  ...
                  <p className="text-sm">{dir === "rtl" ? "Gemini يقرأ الوثيقة..." : "Gemini is reading the document..."}</p>
                </div>
              )}
            ...
            <div className="border-t bg-white px-6 py-5 space-y-3 shrink-0">
              ...
              <div className="space-y-1.5">
                ...
                <textarea
                  ...
                  className={cn(
                    ...
                    "resize-none focus:outline-none focus:border-blue-400 transition-colors",
              ...
              >
                {isRunning
                  ...
                  : <><Zap className="w-4 h-4" />{dir === "rtl" ? "استخراج بـ Gemini" : "Extract with Gemini"}</>
                }
              </button>
              ...
              {phase === "done" && result?.text && (
                <button
                  onClick={handleCopy}
                  className="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl border text-sm text-muted-foreground hover:text-foreground hover:border-blue-400/40 transition-colors"
                >
                  {copied
