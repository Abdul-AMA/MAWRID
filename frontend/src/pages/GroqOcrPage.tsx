...
import { runGroqOcr, getGroqModels, VisionOcrResult } from "@/lib/api";
import {
  Upload, Loader2, FileText, RotateCcw, Copy, Check, Zap, Coins,
} from "lucide-react";
...
export default function GroqOcrPage({ dir }: Props) {
  ...
  const { data: modelsData } = useQuery({
    ...
    queryFn: () => getGroqModels().then(r => r.data.models),
    staleTime: Infinity,
  });
  ...
  const mutation = useMutation({
    mutationFn: (f: File) => runGroqOcr(f, model, prompt).then(r => r.data),
  ...
  const shortName = (m: string) => m.includes("/") ? m.split("/")[1] : m;
            ...
            <div className="h-12 px-4 border-b bg-white flex items-center gap-2 shrink-0">
              ...
              {result && (
                <span className="text-xs text-muted-foreground shrink-0 font-mono">
                  {shortName(result.model)} · {result.latency_ms} ms
                </span>
              )}
            </div>
            ...
            <div className="flex-1 overflow-y-auto bg-muted/20">
              ...
              {phase === "running" && (
                <div className="flex flex-col items-center justify-center h-full gap-3 text-muted-foreground py-20">
                  ...
                  <p className="text-sm">{dir === "rtl" ? "Groq يعالج الوثيقة..." : "Groq is processing..."}</p>
                </div>
              )}
          ...
          <div className="flex-1 flex items-center justify-center p-8">
            <label
              className={cn(
                ...
                dragging
                  ? "border-violet-500 bg-violet-50 scale-[1.03] shadow-lg"
                  : "border-border hover:border-violet-400/60 hover:bg-muted/30 hover:shadow-sm"
              )}
            ...
            >
              <div className={cn(
                ...
                dragging ? "bg-violet-100 scale-110" : "bg-muted"
              )}>
                <Upload className={cn("w-10 h-10 transition-colors", dragging ? "text-violet-500" : "text-muted-foreground")} />
              </div>
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-3">
              ...
              {phase === "running" && (
                <div className="flex flex-col items-center justify-center h-40 gap-3 text-muted-foreground">
                  ...
                  <p className="text-sm">
                    {dir === "rtl"
                      ? `${shortName(model)} يقرأ الوثيقة...`
                      : `${shortName(model)} is reading...`}
                  </p>
                </div>
              )}
            ...
            <div className="border-t bg-white px-6 py-5 space-y-3 shrink-0">
              ...
              <div className="space-y-1.5">
                ...
                <textarea
                  ...
                  className={cn(
                    ...
                    "resize-none focus:outline-none focus:border-violet-400 transition-colors",
              ...
              >
                {isRunning
                  ...
                  : <><Zap className="w-4 h-4" />{dir === "rtl" ? "استخراج بـ Groq" : "Extract with Groq"}</>
                }
              </button>
              ...
              {phase === "done" && result?.text && (
                <button
                  onClick={handleCopy}
                  className="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl border text-sm text-muted-foreground hover:text-foreground hover:border-violet-400/40 transition-colors"
                >
                  {copied
