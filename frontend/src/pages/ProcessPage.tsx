...
import {
  getTwoStageModels, runTwoStage, getSchema, saveDocument,
...
function FieldInput({
  ...
  field: UIField; value: string; onChange: (v: string) => void; disabled?: boolean;
}) {
  const base = cn(
...
// ── Stage 4: Review & Edit Fields ─────────────────────────────────────────────

function Stage4Card({
  stage3Done, docFields, fieldValues, onFieldChange, disabled, dir,
}: {
  stage3Done: boolean; docFields: UIField[]; fieldValues: Record<string, string>;
  onFieldChange: (id: string, v: string) => void; disabled: boolean; dir: "rtl" | "ltr";
}) {
  return (
    <div className={cn(
      "rounded-2xl border-2 bg-white transition-all duration-500",
      stage3Done ? "border-violet-200" : "border-border opacity-50",
    )}>
      <StageHeader
        num={4} icon={Pencil} color="bg-violet-50"
        title={dir === "rtl" ? "مراجعة الحقول وتعديلها" : "Review & Edit Fields"}
        subtitle={dir === "rtl" ? "تعديل النتائج المستخرجة إذا لزم الأمر" : "correct extracted results if needed"}
        done={false} loading={false}
      />
      <div className="p-5">
        {!stage3Done && <Empty label={dir === "rtl" ? "في انتظار المرحلة الثالثة" : "Waiting for Stage 3"} />}
        {stage3Done && docFields.length === 0 && (
          <p className="text-sm text-muted-foreground text-center py-6">
            {dir === "rtl" ? "نوع الوثيقة غير موجود في المخطط" : "Document type not found in schema"}
          </p>
        )}
        {stage3Done && docFields.length > 0 && (
          <div className="space-y-3">
...
function Stage5Card({
  active, saved, onSave, isSaving, dir,
}: {
  active: boolean; saved: boolean; onSave: () => void; isSaving: boolean; dir: "rtl" | "ltr";
}) {
  return (
    <div className={cn(
      "rounded-2xl border-2 bg-white transition-all duration-500",
      saved ? "border-emerald-200" : active ? "border-sky-200" : "border-border opacity-50",
    )}>
      <StageHeader
        num={5} icon={Save} color={saved ? "bg-emerald-50" : "bg-sky-50"}
        title={dir === "rtl" ? "حفظ السجل" : "Save Record"}
        subtitle={dir === "rtl" ? "حفظ البيانات في قاعدة البيانات" : "persist reviewed data to the database"}
        done={saved} loading={isSaving}
      />
      <div className="p-5">
        {!active && !saved && (
          <Empty label={dir === "rtl" ? "في انتظار مراجعة الحقول" : "Waiting for field review"} />
        )}
        {active && !saved && (
          <button
            onClick={onSave}
        ...
        {saved && (
          <div className="flex items-center justify-center gap-2 py-6 text-emerald-700 text-sm font-medium animate-in fade-in duration-300">
...
export default function ProcessPage({ dir }: Props) {
  ...
  // Derive schema fields for the detected doc type
  const detectedDocType  = result?.stage2?.document_type ?? null;
  const docSchemaEntry   = detectedDocType ? schemaData?.documents[detectedDocType] : null;
  const docFields: UIField[] = docSchemaEntry
    ? docSchemaEntry.fields.map(f => ({
        id:       f.id,
        labelAr:  f.label_ar,
        type:     f.type,
        required: f.required,
  ...
  // Pre-populate field values when stage 3 completes
  useEffect(() => {
    if (!result?.stage3?.fields) return;
    const vals: Record<string, string> = {};
    Object.entries(result.stage3.fields).forEach(([k, v]) => {
  ...
  const saveMutation = useMutation({
    mutationFn: () => {
      const catId    = docSchemaEntry?.category ?? "";
      const catLabel = schemaData?._categories?.[catId]?.label_ar ?? docSchemaEntry?.category_label_ar ?? catId;
      const totalMs  = (result?.stage1.latency_ms ?? 0) + (result?.stage2.latency_ms ?? 0) + (result?.stage3.latency_ms ?? 0);
      const confMap: Record<string, number> = { high: 1.0, medium: 0.5, low: 0.0 };
      const confidence = confMap[result?.stage2.confidence ?? "low"] ?? 0;
      return saveDocument({
        file:          file!,
        category:      catId,
        categoryLabel: catLabel,
        docType:       detectedDocType!,
        docTypeLabel:  result?.stage2.document_type_label ?? detectedDocType!,
  ...
  const handleReset = () => {
    ...
    setPhase("idle"); setError(null); setFieldValues({}); setSaved(false);
  };

  const isRunning  = phase === "running";
  const stage3Done = !!result?.stage3 && !isRunning;
  const canSave    = stage3Done && !!detectedDocType && !saved;
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-1">
              ...
              <div className="flex items-center gap-2 mb-4">
                ...
                <h2 className="text-sm font-semibold text-muted-foreground uppercase tracking-wider ms-1">
                  {dir === "rtl" ? "خط أنابيب خمسي المراحل" : "Five-Stage Pipeline"}
                </h2>
                {result && <span className="ms-auto"><ModelChip backend={model} /></span>}
              </div>

              {phase === "error" && (
                <div className="p-4 rounded-xl bg-destructive/10 text-destructive text-sm mb-2">
              ...
              <Connector
                active={stage3Done}
                label={dir === "rtl" ? "الحقول المستخرجة للمراجعة" : "extracted fields for review"}
              />

              <Stage4Card
                stage3Done={stage3Done}
                docFields={docFields}
              ...
              <Connector
                active={canSave || saved}
                label={dir === "rtl" ? "حقول مراجعة ومعدّلة" : "reviewed & corrected fields"}
              />
            ...
            <div className="border-t bg-white px-6 py-4 space-y-3 shrink-0">
              <div className="space-y-1.5">
                <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">
                  {dir === "rtl" ? "النموذج (للمراحل الثلاث الأولى)" : "Model (first 3 stages)"}
                </p>
              ...
              >
                {isRunning
                  ...
                  : <><Eye className="w-4 h-4" />{dir === "rtl" ? "تشغيل خط الأنابيب" : "Run Pipeline"}</>
                }
              </button>
            </div>
          </div>
